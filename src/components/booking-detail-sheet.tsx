...
export function BookingDetailSheet({ booking, onClose, onOpenGuest }: BookingDetailSheetProps) {
  ...
  const [showAdvanceRestore, setShowAdvanceRestore] = useState(false);
  ...
  const isGroupBooking = currentBooking?.reservationType === 'group' || !!currentBooking?.groupId;

  const leadGuest = useMemo(() => {
    if (!isGroupBooking || !currentBooking?.groupId) return null;
    const allGroupBookings = bookings.filter(b => b.groupId === currentBooking.groupId);
    if (allGroupBookings.length === 0) return null;
    const earliest = [...allGroupBookings].sort((a, b) => new Date(a.createdAt || 0).getTime() - new Date(b.createdAt || 0).getTime())[0];
    if (typeof earliest.guestId === 'object') return earliest.guestId;
    return guests.find(g => g._id === earliest.guestId) || null;
  }, [isGroupBooking, currentBooking, bookings, guests]);
  ...
  useEffect(() => {
    if (showBalanceSettle && activeStats?.balance) {
      setSettleAmount(activeStats.balance);
    }
  }, [showBalanceSettle, activeStats?.balance]);
              ...
              <div className={cn("p-5 sm:p-6 space-y-6", isBlock ? "lg:col-span-5" : "lg:col-span-3")}>
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <h2 className="text-xl font-black text-slate-900 leading-tight">{isBlock ? 'Block' : isEnquiry ? 'Enquiry' : 'Booking'} Details</h2>
                    {bookingData.createdAt && (
                      <p className="text-[10px] font-bold text-slate-400">
                        Handled by <span className="text-slate-600">{bookingData.createdBy?.name || 'System / Direct'}</span> on {format(new Date(bookingData.createdAt), "MMM dd, yyyy 'at' hh:mm a")}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2.5">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                        {isGroupBooking && guest._id !== leadGuest?._id ? 'Room Occupant' : 'Primary Guest'}
                      </h3>
                    ...
                    {isGroupBooking && leadGuest && guest._id !== leadGuest._id && (
                      <div className="space-y-2.5 mt-2">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">Lead Contact (Booking Main)</h3>
                        <button onClick={() => leadGuest._id && onOpenGuest && onOpenGuest(leadGuest._id)} className="w-full p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex items-center gap-4 text-left transition-all hover:border-indigo-200">
                          <div className="h-11 w-11 rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center"><Users className="h-5 w-5" /></div>
                          <div>
                            <p className="font-black text-slate-900 text-sm">{leadGuest.name}</p>
                            <div className="flex flex-col">
                              <p className="text-[11px] text-slate-500 font-bold">{leadGuest.phone}</p>
                              {leadGuest.email && <p className="text-[10px] text-indigo-600/80 font-medium lowercase italic">{leadGuest.email}</p>}
                            </div>
                          </div>
                          <ChevronRight className="ml-auto h-4 w-4 text-slate-300" />
                        </button>
                  ...
                  <div className="space-y-6 flex-1">
                    <div>
                      ...
                      <div className="rounded-[22px] bg-white border border-slate-100 shadow-xl p-5 space-y-5">
                        <div className="space-y-4">
                          {/* Room Level Stats (Individual) */}
                          <div className={cn("p-4 rounded-xl border border-slate-50", isGroupBooking ? "bg-slate-50/50" : "bg-white")}>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-[10px] font-black uppercase text-slate-400">Individual Room</span>
                              {isGroupBooking && <Badge variant="outline" className="text-[9px] bg-white border-slate-200">This Room Only</Badge>}
                            </div>
                            <div className="flex items-baseline gap-2">
                              <span className={cn("text-2xl font-black tabular-nums", priceStats.balance <= 0 ? 'text-emerald-500' : 'text-slate-900')}>₹{priceStats.balance.toLocaleString('en-IN')}</span>
                            ...
                            <div className="flex justify-between text-[9px] font-bold text-slate-400 mt-1 uppercase">
                              <span>Paid: ₹{priceStats.advancePayment.toLocaleString('en-IN')}</span>
                              <span>Total: ₹{priceStats.totalAmount.toLocaleString('en-IN')}</span>
                            </div>
                          </div>

                          {/* Group Level Stats (Together) */}
                          {isGroupBooking && groupPriceStats && (
                            <div className="p-4 rounded-xl bg-indigo-600 text-white shadow-lg shadow-indigo-100">
                              <div className="flex justify-between items-center mb-2">
                                <span className="text-[10px] font-black uppercase text-indigo-100">Group Summary</span>
                                <Badge className="text-[9px] bg-indigo-500 text-white border-indigo-400">All {sortedGroupRooms.length} Rooms</Badge>
                              </div>
                              <div className="flex items-baseline gap-2">
                                <span className={cn("text-2xl font-black tabular-nums", groupPriceStats.balance <= 0 ? 'text-emerald-400' : 'text-white')}>₹{groupPriceStats.balance.toLocaleString('en-IN')}</span>
                                <span className="text-[10px] font-bold text-indigo-200 uppercase tracking-widest">Due</span>
                              </div>
                              <div className="flex justify-between text-[9px] font-bold text-indigo-100 mt-1 uppercase">
                                <span>Paid: ₹{groupPriceStats.advancePayment.toLocaleString('en-IN')}</span>
                                <span>Total: ₹{groupPriceStats.totalAmount.toLocaleString('en-IN')}</span>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        {bookingData.status !== 'cancelled' && bookingData.status !== 'checked-out' && activeStats.balance > 0 && (
                          <div className="space-y-2">
                            {!showBalanceSettle ? (
                              ...
                              <div className="flex flex-col gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
                                <div className="flex items-center gap-2">
                                  <IndianRupee className="h-4 w-4 text-slate-400" />
                                  <Input
                                    type="number"
                                    className="h-9 font-bold bg-white text-sm"
                                    value={settleAmount || ''}
                                ...
                                <div className="grid grid-cols-3 gap-2 mt-1">
                                  {['Cash', 'Card', 'UPI'].map(m => (
                                    <Button key={m} size="sm" variant="outline" className="text-[10px] font-black h-8" onClick={() => handleAction(id => updateBooking(id, { advancePayment: (bookingData.advancePayment || 0) + (settleAmount || activeStats.balance), paymentMethod: m.toLowerCase() as any }), true)}>{m}</Button>
                                  ))}
                                </div>
                              </div>
                            )}
            ...
            <div className="flex-1 flex gap-2">
              {bookingData.status === 'reserved' && !isEnquiry && (
                <div className="flex-1 flex flex-col gap-1">
                  <Button
                    className="w-full h-11 rounded-xl bg-indigo-600 text-white font-black uppercase text-[11px] tracking-widest shadow-lg shadow-indigo-100 disabled:opacity-50 disabled:grayscale"
                    onClick={handleInitialCheckIn}
                    disabled={isActioning || startOfDay(new Date(bookingData.checkin)) > startOfDay(new Date())}
                  >
                    {startOfDay(new Date(bookingData.checkin)) > startOfDay(new Date()) ? 'Early (Check-in tomorrow)' : 'Secure Check-in'}
                  </Button>
                  {startOfDay(new Date(bookingData.checkin)) > startOfDay(new Date()) && (
                    <p className="text-[9px] font-bold text-amber-600 uppercase tracking-widest text-center animate-pulse">
                      Check-in allowed from {format(new Date(bookingData.checkin), 'dd MMM')}
                    </p>
                  )}
                </div>
              )}
              {bookingData.status === 'checked-in' && (
                <div className="flex-1 flex gap-2">
                  {!showBalanceSettle && activeStats.balance > 0 ? (
                    ...
                    <div className="flex-1 flex items-center gap-2 p-1 bg-slate-100 rounded-xl animate-in fade-in zoom-in-95 duration-200">
                      <div className="relative w-28 shrink-0 hidden sm:block">
                        <IndianRupee className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
                        <Input
                          type="number"
                          className="h-9 pl-7 pr-2 font-bold bg-white text-xs rounded-lg"
                          value={settleAmount || ''}
                          onChange={e => setSettleAmount(Number(e.target.value))}
                          placeholder="Amount"
                        />
                      ...
                      <div className="flex flex-1 gap-1.5">
                        {['Cash', 'Card', 'UPI'].map(m => (
                          <Button
                            key={m}
                            className="flex-1 h-9 rounded-lg bg-white text-slate-900 border-none shadow-sm hover:bg-orange-50 hover:text-orange-600 font-bold text-[10px] uppercase tracking-wider px-1"
                            onClick={() => handleAction(id => updateBooking(id, {
                              advancePayment: (bookingData.advancePayment || 0) + (settleAmount || activeStats.balance),
                      ...
                      <Button variant="ghost" size="icon" className="h-9 w-9 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 shrink-0" onClick={() => setShowBalanceSettle(false)}><X className="h-4 w-4" /></Button>
                    </div>
                  ) : (
                    <Button
                      className="flex-1 h-11 rounded-xl bg-orange-600 text-white font-black uppercase text-[11px] tracking-widest shadow-lg shadow-orange-100"
              ...
              {isEnquiry && isEnquiryExpired && (
                <div className="flex-1 flex gap-2">
                  {!showAdvanceRestore ? (
                    <Button className="flex-1 h-11 rounded-xl bg-cyan-600 text-white font-black uppercase text-[11px] shadow-lg shadow-cyan-100" onClick={() => setShowAdvanceRestore(true)} disabled={isActioning}>
                      Restore & Confirm
                    </Button>
                  ) : (
                    <div className="flex-1 flex items-center gap-2 p-1 bg-slate-100 rounded-xl animate-in fade-in zoom-in-95 duration-200">
                      ...
                      <div className="flex flex-1 gap-1.5">
                        {['Cash', 'Card', 'UPI'].map(m => (
                          <Button
                            ...
                            className="flex-1 h-9 rounded-lg bg-white text-slate-900 border-none shadow-sm hover:bg-cyan-50 hover:text-cyan-600 font-bold text-[10px] uppercase tracking-wider px-1 disabled:opacity-50"
                            onClick={() => {
                              setShowAdvanceRestore(false);
                              handleAction(id => updateBooking(id, {
                                ...
                                bookingType: 'booking',
                                status: 'reserved',
                                advancePayment: (bookingData.advancePayment || 0) + (settleAmount || 0),
                                paymentMethod: m.toLowerCase() as any,
                              }));
                            }}
                          >
                      ...
                      <Button variant="ghost" size="icon" className="h-9 w-9 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 shrink-0" onClick={() => setShowAdvanceRestore(false)}><X className="h-4 w-4" /></Button>
                    </div>
                  )}
                </div>
              )}
